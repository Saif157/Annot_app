...
with st.sidebar:
    ...
    # Progress indicator
    progress_value = st.session_state.current_image_index / max(len(st.session_state.uploaded_files) - 1, 1)
        ...
        if objects:
            for i, obj in enumerate(objects):
                ...
                with col2:
                    ...
                    confidence = obj.get("confidence", "")
                    conf_text = f" ({confidence:.3f})" if confidence else ""
                    st.write(f"{obj_type} - {obj_class}{conf_text}")
                with col3:
                    if st.button("🗑️", key=f"delete_{i}", help="Delete this object"):
                        objects.pop(i)
